...
- (void)addCustomAction
{
    ...
    CustomJSObject *object = [[CustomJSObject alloc]initWithSuccessCallback:^(NSDictionary *dic) {
        NSLog(@"oc响应 %@",dic);
        if ([[dic.allKeys firstObject] isEqualToString:@"sendValueFromHtmlToOCWithValueWithValueTwo"]){
         //if ([[dic.allKeys firstObject] isEqualToString:@"login"]){
            //从html传两个值给OC
            NSLog(@"login参数:%@",dic);
        }
        ...
        if ([[dic.allKeys firstObject] isEqualToString:@"login"]){
               //从html传两个值给OC
            NSString *param = dic[@"login"][@"valueOne"];
            NSDictionary *retDict = [self dictionaryWithJsonString:param];
            NSLog(@"login参数:%@",retDict);
            //   NSLog(@"login参数:%@",dic);
        }else if ([[dic.allKeys firstObject] isEqualToString:@"deviceInfo"]){
            //从OC传值给html
            NSLog(@"deviceInfo:%@",dic);

            NSString *deviceName = @"iphone 12";
            NSString *account = @"admin";
            NSString *textJS = [NSString stringWithFormat:@"deviceInfo('%@')",deviceName];
            [context evaluateScript:textJS];
        }
